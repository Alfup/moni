...
export function useGlslCanvas({ canvas: canvasArg, ...materialOptions }: Args) {
   ...
   // Cast the uniforms to CustomUniforms type after ensuring it has the required properties
   const uniforms = mesh.material.uniforms as unknown as CustomUniforms;
   ...
   // Make canvas interactive for mobile but don't block scrolling
   canvas.style.pointerEvents = 'auto';
   
   // Add loaded class to show the canvas
   canvas.classList.add("loaded");

   return {
      renderer,
      raf,
      uniforms: mesh.material.uniforms as unknown as CustomUniforms,
      canvas,
   };
}