export interface CustomUniforms {
  ...
  uPlanetPosition: [number, number, number];
  uCameraPosition: [number, number, number];
  uCameraView: number[];
  uRotationOffset: number;
  uPlanetRadius: number;
  uBumpStrength: number;
  uNoiseStrength: number;
  uTerrainScale: number;
  uCloudsDensity: number;
  uCloudsScale: number;
  uCloudsSpeed: number;
  uAtmosphereColor: [number, number, number];
  uAtmosphereDensity: number;
  uAmbientLight: number;
  uSunIntensity: number;
  sunDirectionXY: [number, number];
  uPlanetColor: any;
}
